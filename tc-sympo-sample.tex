...
\subsection{ハードウェア}
本チームは屋外自律移動ロボットとして, ORNE-α, ORNE-box, ORNE-box2の開発を行っており, つくばチャレンジ2022にはこれらのロボットが参加する. \figref{fig:matching-concept}に本チームの開発している自律移動ロボットの外観を示す. これらのロボットはi-Cart middleをベースとしており, 主なセンサはIMU, 測域センサである. 
...
\begin{figure}[h]
  ...
  \end{minipage}
  \caption{ORNE-Series}
  \label{fig:matching-concept}%\vspace*{-2mm}
\end{figure}
\begin{figure*}
  \centering
  ...
  \label{fig:icp-comp_viz_hall_normal}%\vspace*{-2mm}
\end{figure*}
...
\section{図}
\label{sec:fig_main}
本章では、図を挿入する複数の方法、キャプションの書き方や本文中での参照の仕方について述べる。

\subsection{Figure の挿入}
\figref{fig:matching-concept}に、図を挿入する基本形を示す。
図の挿入には、figure 環境（\verb*|\begin{figure}|、\verb*|\end{figure}|）と \verb*|\include| \verb*|graphics{}| コマンドを用いる。
具体的な書き方は \LaTeX ソースを参照されたい。

\figref{fig:icp-comp_viz_hall_normal}のように2カラムで2段抜きの図を挿入する場合、figure* 環境（\verb*|\begin{figure*}|、\verb*|\end| \verb*|{figure*}|）を用いる。
